...
#include "gen-cpp/Bonk.h"
#include "gen-cpp/EmptyStruct.h"
#include "gen-cpp/ListBonks.h"
#include "gen-cpp/OneField.h"
#include "gen-cpp/StructA.h"
#include "gen-cpp/Tricky2.h"

using apache::thrift::to_string;

BOOST_AUTO_TEST_SUITE(ToStringTests)
...
BOOST_AUTO_TEST_CASE(empty_boost_optional_serializes_to_null) {
  boost::optional<int> i;
  BOOST_CHECK_EQUAL(to_string(i), "<null>");
}

BOOST_AUTO_TEST_CASE(initialized_boost_optional_serializes_inner_value) {
  boost::optional<int> i = 2;
  BOOST_CHECK_EQUAL(to_string(i), "2");
}

enum class EnumClass { Value = 123 };
BOOST_AUTO_TEST_CASE(enum_class_serializes_underlying_type) {
  BOOST_CHECK_EQUAL(to_string(EnumClass::Value), "123");
}
...
BOOST_AUTO_TEST_CASE(single_item_unordered_set_to_string) {
  ...
  s.insert("xyz");
  BOOST_CHECK_EQUAL(to_string(s), "{xyz}");
}
