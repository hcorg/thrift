...
void TOverlappedWorkItem::reset(uint8_t* buf, std::uint32_t len, HANDLE event) {
...
std::uint32_t TOverlappedWorkItem::overlappedResults(bool signal_failure) {
  DWORD bytes = 0;
...
std::uint32_t TOverlappedSubmissionThread::instanceRefCount_ = 0;
}
}
