...
public:
   ...
   */
  virtual void freeContext(void* ctx) { (void)ctx; }
   ...
   */
  virtual void postRead(void* ctx, const char* fn_name, std::uint32_t bytes) {
    (void)ctx;
    (void)fn_name;
    (void)bytes;
  }
   ...
   */
  virtual void postWrite(void* ctx, const char* fn_name, std::uint32_t bytes) {
    (void)ctx;
    (void)fn_name;
    (void)bytes;
  }
...
public:
  TProcessorContextFreer(TProcessorEventHandler* handler, void* context)
    : handler_(handler), context_(context) {}
  ~TProcessorContextFreer() {
    if (handler_)
      handler_->freeContext(context_);
  }

private:
  TProcessorEventHandler* handler_;
  void* context_;
};
...
public:
  ...
  bool process(std::shared_ptr<protocol::TProtocol> io, void* connectionContext) {
...
public:
  ReleaseHandler(const std::shared_ptr<HandlerFactory>& handlerFactory)
    : handlerFactory_(handlerFactory) {}

  void operator()(typename HandlerFactory::Handler* handler) {
    if (handler) {
...
private:
  std::shared_ptr<HandlerFactory> handlerFactory_;
};
