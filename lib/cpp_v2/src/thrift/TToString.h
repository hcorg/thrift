...
template <typename T>
typename std::enable_if<!std::is_enum<T>::value, std::string>::type to_string(const T& t) {
  std::ostringstream s;
  s << t;
  return s.str();
}
...
template <typename T>
typename std::enable_if<std::is_enum<T>::value, std::string>::type to_string(T t) {
  return to_string(static_cast<typename std::underlying_type<T>::type>(t));
}
