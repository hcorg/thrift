...
bool TFileTransport::initBufferAndWriteThread() {
  ...
  if (!writerThread_.joinable()) {
    writerThread_ = std::thread([this]() { writerThread(); });
  }
...
void TFileTransport::enqueueEvent(const uint8_t* buf, uint32_t eventLen) {
  ...
  notFull_.wait(lock, [this]() { return !enqueueBuffer_->isFull(); });
...
bool TFileTransport::swapEventBuffers(std::chrono::system_clock::time_point deadline) {
  bool swap;
  std::unique_lock<std::mutex> lock(mutex_);
...
void TFileTransport::writerThread() {
  ...
  if (!hasIOError) {
    try {
      ...
      if (THRIFT_FTRUNCATE(fd_, offset_) != 0) {
        int errno_copy = THRIFT_ERRNO;
        GlobalOutput.perror("TFileTransport: writerThread() - file truncation ", errno_copy);
      }
  ...
  auto tsNextFlush = getNextFlushTime();
  ...
  while (1) {
    ...
    if (swapEventBuffers(tsNextFlush)) {
      eventInfo* outEvent;
      while (NULL != (outEvent = dequeueBuffer_->getNext())) {
        ...
        if ((outEvent->eventSize_ > 0) && (chunkSize_ != 0)) {
          ...
          if (chunk1 != chunk2) {
            ...
            std::unique_ptr<uint8_t[]> zeros(new uint8_t[padding]);
            memset(zeros.get(), '\0', padding);
            if (-1 == ::write(fd_, zeros.get(), padding)) {
              int errno_copy = THRIFT_ERRNO;
...
void TFileTransport::flush() {
  ...
  notEmpty_.notify_all();
  lock.lock();
  flushed_.wait(lock, [this]() { return !forceFlush_; });
}
...
void TFileTransport::seekToChunk(int32_t chunk) {
  ...
  if (seekToEnd) {
    ...
    std::unique_ptr<eventInfo> event;
...
std::chrono::system_clock::time_point TFileTransport::getNextFlushTime() {
  auto curTs = std::chrono::system_clock::now();
  curTs += std::chrono::microseconds(flushMaxUs_);
  return curTs;
}
