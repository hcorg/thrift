...
uint32_t TDebugProtocol::startItem() {
  ...
  case MAP_VALUE:
    return writePlain(" -> ");
  case LIST:
    size = writeIndented("[" + std::to_string(list_idx_.back()) + "] = ");
    list_idx_.back()++;
    return size;
  default:
...
uint32_t TDebugProtocol::writeFieldBegin(const char* name,
  ...
  string id_str = std::to_string(fieldId);
  if (id_str.length() == 1)
    id_str = '0' + id_str;
...
uint32_t TDebugProtocol::writeString(const string& str) {
  ...
  if (to_show.length() > (string::size_type)string_limit_) {
    ...
    to_show += "[...](" + std::to_string(str.length()) + ")";
  }
