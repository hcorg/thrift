...
struct OperatorLtGeneratorTestsFixture {

  OperatorLtGeneratorTestsFixture() : tstruct(nullptr) {}

  std::string generateBody() {
    std::ostringstream stream;
    OutputGenerator parent(stream);
    OperatorLtGenerator gen(parent, &tstruct);
...
BOOST_FIXTURE_TEST_SUITE(OperatorLtGeneratorTests, OperatorLtGeneratorTestsFixture)
...
BOOST_AUTO_TEST_CASE(empty_struct_always_returns_false) {
  BOOST_CHECK_EQUAL(generateBody(), "return false;\n");
}
